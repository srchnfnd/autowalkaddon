...
float PENALTY_NOTIFY_THRESHOLD = 10.0 const
...
Function PrepareForWalking()
	...
	Debug.Trace("AutoWalk: PrepareForWalking(): bUseAnimationFootstepCheck=" + bUseAnimationFootstepCheck, 1)
	if bUseAnimationFootstepCheck == false
		ResetFootstepCheckResults()
	endif
	
	; 발자국 이벤트가 지원되면 모니터링 기준을 더 안정적으로 잡고,
	; 지원되지 않으면 카메라/이동 상태를 강제로 맞춘 뒤 단순한 jitter 대응만 사용한다.
	; 모든 속도 체크가 끝나고 모두 지원되지 않을 때만 카메라 강제 동기화.
...
Function StartMonitoring()
	...
	CancelTimer(TIMER_MONITOR_JITTER)
	; ResetPathing, PrepareForWalking에서 걸어논 타이머를 덮어놓고 캐슬시키면 안된다
	; CancelTimer(TIMER_RESTORE_WALK_SPEED)
	; CancelTimer(TIMER_RESTORE_3RD_PERSON)
...
int EXECUTE_SAMPLE_RESULT_FOOTSTEP_CHECKS_COMPLETE = 3 const
...
MonitorResult Function ExecuteSample(int walkSpeed, bool bIsSneaking, bool bIsRunning)
	...
	int footstepResultCode = _ProcessFootstepCheck(walkSpeed, bIsRunning, distancePerSec, yawDeltaPerSec, leftRatePerSec, rightRatePerSec)
...
var[] Function _CheckFootstep(float leftRatePerSec, float rightRatePerSec, float minExpectedFootstepRate)
	...
	if Utility.GetCurrentRealTime() - fFootstepCheckStartTime >= fFootstepCheckDuration
		result[0] = true
		if leftRatePerSec >= minExpectedFootstepRate && rightRatePerSec >= minExpectedFootstepRate
			result[1] = true
			...
			result[1] = false
			Debug.Notification("AutoWalk: [_CheckFootstep] " + walkRunAnimationName + " animation does not support footstep event.")
		endif
	endif
...
int Function _ProcessFootstepCheck(int walkSpeed, bool bIsRunning, float distancePerSec, float yawDeltaPerSec, float leftRatePerSec, float rightRatePerSec)
	if iFootstepCheckPhase == 0
	...
	if iFootstepCheckPhase == 1
		...
		if distancePerSec >= minDistance && yawDeltaPerSec < 10.0
			if bIsRunning == GetExpectedRunning(walkSpeed)
				...
				Debug.Notification("AutoWalk: Checking for animation footstep, please wait...")
	...
	if iFootstepCheckPhase == 2
		if !IsFootstepCheckDone(walkSpeed)
			...
			if footstepCheckResult[0]
				...
				Debug.Notification("AutoWalk: Footstep check for " + GetFootstepAnimationName(walkSpeed) + " completed. Result: " + resultStr)
				fFootstepCheckStartTime = 0.0
